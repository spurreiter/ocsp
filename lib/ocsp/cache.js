...
Cache.prototype.probe = function probe (id, callback) {
  if (Object.prototype.hasOwnProperty.call(this.cache, id)) { callback(null, this.cache[id]) } else { callback(null, false) }
}

Cache.prototype.store = function store (id, response, maxTime, callback) {
  if (Object.prototype.hasOwnProperty.call(this.cache, id)) { clearTimeout(this.cache[id].timer) }
  var self = this
  this.cache[id] = {
    response: response,
